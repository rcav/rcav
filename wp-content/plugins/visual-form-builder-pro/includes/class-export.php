...
?>
        		...
        		<li>
        			...
        			<p><a id="vfb-export-select-all" href="#"><?php _e( 'Select All', 'visual-form-builder-pro' ); ?></a></p>
	...
	<Worksheet ss:Name="Sheet1">
		<Table x:FullColumns="1" x:FullRows="1" ss:DefaultColumnWidth="65" ss:DefaultRowHeight="15">
			...
			<?php foreach ( $fields_header as $field ) : ?>
				<Cell><Data ss:Type="String"><?php echo esc_html( wp_specialchars_decode( $field, ENT_QUOTES ) ); ?></Data></Cell>
			<?php endforeach; ?>
			</Row>
		<?php
		// Build table rows and cells
		foreach ( $data as $row ) :
			...
			foreach ( $fields_clean as $label ) {
				...
				if ( isset( $row[ $label ] ) && in_array( $label, $fields ) ) {
					...
					echo sprintf( '<Cell%3$s><Data ss:Type="%2$s">%1$s</Data></Cell>', $item, $type, $style );
				}
				else
