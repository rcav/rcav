...
class PMXI_Chunk {
  ...
  public function read() {
      ...
      while ($this->reading and (count($founded_tags) < 500)) {
        ...
        if ( @preg_match_all("/<\\w+\\s*[^<|^\n]*\\s*\/?>/" . ($this->case_sensitive ? "i" : ""), $c, $matches, PREG_PATTERN_ORDER) ){
          foreach ($matches[0] as $tag) {
            if ( strpos($tag, "<br") === false and strpos($tag, "?xml") === false and strpos($tag, "!--") === false ) {
    ...
    $main_elements = array('node', 'product', 'job', 'deal', 'entry', 'item', 'property', 'listing', 'hotel', 'record');
    ...
    if (empty($element)){      
      if (!empty($this->cloud)){      
        ...
        if (empty($element))
          $this->options['element'] = $element = array_shift($element_counts);        
      }      
      else return false;
    }                              
    ...
    while ($this->reading && !@feof($this->handle)) {            
      ...
      if ($checkOpen === false) {
        ...
        //var_dump(iconv('UTF-16', 'UTF-8', $tmp));die;
